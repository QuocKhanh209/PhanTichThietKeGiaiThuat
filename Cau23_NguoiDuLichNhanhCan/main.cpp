...
//1... max+1
int BestWay[MAX + 2]; //Mang BestWay de ghi nhan nghiem
...
void PrintResult()
{
    ...
    {
    	output<<"- Hanh trinh ngan nhat: ";
        for (int i = 1; i <= n; i++)
            output << BestWay[i] << "->";
        output << "1" << endl;
        output << "- Chi phi it nhat : " << minSpending;
    }
    output.close();
}
...
void Try(int i) 
{
    for (int j = 2; j <= n; j++) //thu cac tp tu 2 den n
    {
        if (Free[j]) // neu gap thanh pho chua di qua
        {
            ...
            /*
            Chi khac quay lui o buoc nay, thay vi chi kiem tra co duong di truc tiep tu X[x-1]->j
			thi ta kiem tra luon chi phi tu 1->T[i] co < phuong an toi uu toi thoi diem nay khong. 
			Neu khong thi huy luon khong xet nhanh nay nua
            */
            if (T[i] < minSpending)  // Hien nhien co dieu nay thi C[X[i - 1]][j]<INF r�i, co duong di truc tiep tu X[i-1] den j
            {
